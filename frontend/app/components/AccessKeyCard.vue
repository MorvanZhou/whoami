...
<template>
  <div class="group rounded-xl bg-th-bg-s border border-th-text/[var(--color-card-border-opacity)] hover:border-th-text/[0.12] transition-all duration-200" role="article" :aria-label="displayName">
    ...
    <div class="hidden sm:flex items-center justify-between gap-4 px-5 py-4">
      <div class="flex items-center gap-4 min-w-0">
        <div class="shrink-0 w-10 h-10 rounded-lg bg-highlight/8 flex items-center justify-center">
          <svg class="w-4 h-4 text-highlight/60" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div class="min-w-0">
          <div class="flex items-center gap-2.5 mb-1">
            ...
            <span
              ...
              role="status"
            >
              <span class="w-1.5 h-1.5 rounded-full" :class="lastUsed ? 'bg-success' : 'bg-th-text-m'" aria-hidden="true" />
          ...
          <div class="flex items-center gap-3 text-xs text-th-text-t">
            ...
            <template v-if="lastUsed">
              <span class="text-th-text-m" aria-hidden="true">·</span>
      ...
      <button
        class="shrink-0 px-3 py-1.5 rounded-md text-xs font-medium text-th-text-t hover:text-danger hover:bg-danger/8 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 transition-all duration-200"
        :aria-label="`${t('dashboard.unlink')} ${displayName}`"
    ...
    <div class="sm:hidden px-4 py-3.5">
      ...
      <div class="flex items-center gap-2 mb-2">
        <svg class="shrink-0 w-4 h-4 text-highlight/60" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
        </svg>
      ...
      <div class="flex items-center justify-between">
        ...
        <button
          class="shrink-0 ml-3 text-xs font-medium text-th-text-t hover:text-danger transition-colors duration-200"
          :aria-label="`${t('dashboard.unlink')} ${displayName}`"
          @click="emit('delete', id)"
        >
          {{ t('dashboard.unlink') }}
