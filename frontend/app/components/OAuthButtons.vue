...
const githubLogin = () => {
  ...
  window.location.href = `${config.public.apiBase}/auth/github/login?${params.toString()}`
}

const googleLogin = () => {
  const params = new URLSearchParams()
  if (props.redirect) params.set('redirect', props.redirect)
  window.location.href = `${config.public.apiBase}/auth/google/login?${params.toString()}`
}
</script>
