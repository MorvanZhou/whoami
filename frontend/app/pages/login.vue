...
<template>
  <div class="relative min-h-[calc(100vh-4rem)] flex items-center justify-center px-6">
    ...
    <div class="relative z-10 w-full" :class="isFromAgent ? 'max-w-md' : 'max-w-sm'">
      ...
      >
        <div v-if="isFromAgent" class="mb-5 p-5 rounded-2xl bg-accent/[0.04] border border-accent/15 backdrop-blur-sm" role="alert">
          ...
          <div class="flex items-center gap-1.5 mt-3 pt-3 border-t border-th-text/[var(--color-border-opacity)]">
            <svg class="w-4 h-4 text-success/70 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div class="p-8 rounded-2xl bg-th-card/60 border border-th-text/[var(--color-card-border-opacity)] backdrop-blur-xl shadow-2xl">
        ...
        <div class="text-center mb-8">
          <img src="/logo.png" alt="" class="w-14 h-14 mx-auto mb-3" aria-hidden="true" />
          <h1 class="text-accent font-mono font-bold text-2xl">{{ t('login.title') }}</h1>
          <p class="text-sm text-th-text-s mt-2 leading-relaxed">{{ t('login.subtitle') }}</p>
        </div>
